...
{sink("./ranalytics.log")
  ...
  # install.packages(c('data.table','dplyr','tidyverse','bit64','chron','xts','forecast','TSA','gtools','ggplot2','mongolite'))
  ...
  ########### Ecah OLT wise data manipulation, missing data check, outlier check and build model and forecast ################
  for(olt_name in 1:length(OLT)){
    ...
    if(len >= 365){
      ...
      ## Replacing NA values by mean, median 
      ## For Usage
      ...
      for (i in 1:nrow(perms)) {
        ...
        # lapply(df, class)
        nrowof1month <- nrowof1month + nrow(df)
        # cat("\nOLT one month forecast record  :", nrow(df))
      ...
      for (j in 1:nrow(perms)) {
        ...
        colnames(df)<-c("frcstdate","users_growth","users","usage","usage_low","usage_high","period","olt","recorddate")
        nrowof3month<- nrowof3month + nrow(df)
        ## Insert output into DataBase
        con_site_test$insert(df)
      }
      cat("\nOLT one month forecast record  :",  nrowof1month)
      cat("\nOLT three month forecast record  :",  nrowof3month)
      cat("\nTotal number of olt more than 365 days :", count)
      # # cat("\nAccuracy of model :",accuracy(fit.arima))
      end_time <- Sys.time()
      processing_time <- end_time - start_time
      cat("\nOLT run time :", processing_time)
    } else if(len >= 180 & len < 365) {
      count = count + olt_name
      cat("\n\n#### Output of OLT :",OLT[olt_name])
      cat("\nolt name :", OLT[olt_name])
      cat("\nActual length of olt :", length(OLTts$recorddate))
      ...
      cat("\nOLT one month forecast record  :",  nrowof1month)
      cat("\nOLT three month forecast record  :",  nrowof3month)
      cat("\nTotal number of olt more than 180 days and less than 365 days :", count)
      cat("\nAccuracy of model :",accuracy(fit.arima))
      end_time <- Sys.time()
      processing_time <- end_time - start_time
      cat("\nOLT run time :", processing_time)
      ...
      cat("\n\n#### Output of OLT :",OLT[olt_name])
      cat("\nActual length of olt :", length(OLTts$recorddate))
      cat("\nTotal number of olt less than 180 days :", count)
    }
  }
  cat("\n\nTotal OLT :", length(OLT)) # count total OLT
